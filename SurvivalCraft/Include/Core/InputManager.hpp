...
namespace SurvivalCraft
{
	namespace Core
	{
		...
		public:
            ...
            bool GetKey(const KeyCode& code, const KeyState& state)
            {
                return glfwGetKey(window.GetHandle(), static_cast<int>(code)) == static_cast<int>(state);
            }

            bool GetMouseButton(const MouseButton& button, const MouseState& state)
            {
				return glfwGetMouseButton(window.GetHandle(), static_cast<int>(button)) == static_cast<int>(state);
			}
            ...
            {
                static glm::dvec2 lastPosition = GetMousePosition();
                glm::dvec2 currentPosition = GetMousePosition();

                glm::dvec2 delta = currentPosition - lastPosition;
